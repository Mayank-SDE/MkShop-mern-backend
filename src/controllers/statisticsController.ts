...
export const getPieCharts = async (
  ...
  try {
    ...
    if (nodeCache.has(key)) {
      pieCharts = JSON.parse(nodeCache.get(key) as string);
    } else {
      ...
      const discount: number = allOrders.reduce((prev, order) => {
      ...
      const netMargin =
        grossIncome - marketingCost - burnt - productionCost - discount;
